...
class WordStore:
    ...
    def _connect(self):
        ...
        return psycopg.connect(
            self.db_url, user=self.db_user, password=self.db_password, autocommit=False
        )
    ...
    def update_rarity_levels_chunked(
        self, updates: dict[int, int], chunk_size: int = 5000
    ) -> None:
        if not updates:
            return

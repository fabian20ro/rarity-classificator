...
class LmStudioClient:
    def __init__(
        ...
        *,
        ...
        self.response_parser = response_parser or LmStudioResponseParser(
            metrics=metrics
        )
    ...
    def resolve_endpoint(
        self, endpoint_option: str | None, base_url_option: str | None
    ) -> ResolvedEndpoint:
    ...
    def preflight(self, resolved_endpoint: ResolvedEndpoint, model: str) -> None:
        if not resolved_endpoint.models_endpoint:
            return
        resp = self._get_json(
            resolved_endpoint.models_endpoint,
        ...
        if model not in resp.text:
            print(
                f"Warning: model '{model}' not found in {resolved_endpoint.models_endpoint} response."
            )

    def score_batch_resilient(
        self, batch: list[BaseWordRow], context: ScoringContext
    ) -> list[ScoreResult]:
        return self._score_batch_resilient_internal(batch, context, depth=0)

    def _score_batch_resilient_internal(
        self, batch: list[BaseWordRow], ctx: ScoringContext, depth: int
    ) -> list[ScoreResult]:
        if depth >= MAX_RECURSION_DEPTH:
            for word in batch:
                self._log_failed_word(
                    ctx, word, "max_recursion_depth_exceeded", depth=depth
                )
            return []
        ...
        if direct.connectivity_failure:
            raise RuntimeError(
                f"LM request failed due connectivity/timeout at {ctx.endpoint}: {direct.last_error}"
            )
        ...
        if direct.scores:
            ...
            if not direct.unresolved:
                return direct.scores
            retried = self._score_batch_resilient_internal(
                direct.unresolved, ctx, depth + 1
            )
        ...
        if ctx.output_mode == ScoringOutputMode.SELECTED_WORD_IDS:
            ...
            if total_expected is None:
                raise ValueError("expected_json_items is required for selected-id mode")
            left_expected = _compute_split_expected(
                total_expected, len(left_batch), len(batch)
            )
        ...
        return self._score_batch_resilient_internal(
            left_batch, left_ctx, depth + 1
        ) + self._score_batch_resilient_internal(right_batch, right_ctx, depth + 1)

    def _try_score_batch(
        self, batch: list[BaseWordRow], ctx: ScoringContext
    ) -> BatchAttempt:
        ...
        include_reasoning_controls = self._should_include_reasoning_controls(
            ctx.flavor, config
        )
        schema_kind = (
        ...
        resolved_system_prompt, resolved_user_template = (
            self._resolve_selection_prompt_counts(ctx)
        )
        ...
        for attempt in range(ctx.max_retries):
            ...
            try:
                resp = self._post_json(
                    ctx.endpoint, payload, timeout_seconds=ctx.timeout_seconds
                )
                ...
                unsupported_reasoning_controls = (
                    include_reasoning_controls
                    and _is_unsupported_reasoning_controls(exc)
                )
                empty_parsed = (
                    response_format_mode == ResponseFormatMode.JSON_SCHEMA
    ...
    def _try_selection_repair_before_split(
        self, batch: list[BaseWordRow], ctx: ScoringContext
        ...
        print(
            f"Selection repair succeeded (size={len(batch)}, expected={expected}), avoiding recursive split."
        )
    ...
    def _apply_selection_count_placeholders(self, prompt: str, expected: int) -> str:
        return prompt.replace(
            REBALANCE_TARGET_COUNT_PLACEHOLDER, str(expected)
        ).replace(REBALANCE_COMMON_COUNT_PLACEHOLDER, str(expected))
    ...
    def _mark_response_format_json_schema(self) -> None:
        if self.capability_state.response_format_mode != ResponseFormatMode.JSON_SCHEMA:
            self.capability_state = replace(
                self.capability_state,
                response_format_mode=ResponseFormatMode.JSON_SCHEMA,
            )
            print(
                "LM capability: switching response_format to json_schema for this run."
            )

    def _mark_response_format_disabled(self) -> None:
        if self.capability_state.response_format_mode != ResponseFormatMode.NONE:
            self.capability_state = replace(
                self.capability_state, response_format_mode=ResponseFormatMode.NONE
            )
    ...
    def _probe(self, url: str) -> bool:
        try:
            resp = self._get_json(
                url, timeout_seconds=DEFAULT_PREFLIGHT_TIMEOUT_SECONDS
            )
    ...
    def _get_json(self, url: str, *, timeout_seconds: int):
        return self._requests.get(
            url, timeout=timeout_seconds, headers=self._headers(False)
        )
    ...
    def _log_failed_word(
        self, ctx: ScoringContext, word: BaseWordRow, error: str, **extra: Any
    ) -> None:
...
def _compute_split_expected(
    total_expected: int, left_size: int, total_size: int
) -> int:
...
def _should_disable_response_format_after_partial_schema_parse(
    batch_size: int, unresolved_count: int
) -> bool:
    if batch_size <= 0 or unresolved_count <= 0:
        return False
    threshold = max(
        1, int((batch_size * JSON_SCHEMA_UNRESOLVED_DISABLE_RATIO) + 0.9999)
    )
...
def _is_connectivity_failure(exc: Exception) -> bool:
    ...
    if isinstance(
        exc,
        (
            requests_mod.Timeout,
            requests_mod.ConnectionError,
            socket.timeout,
            TimeoutError,
        ),
    ):
    ...
    msg = str(exc).lower()
    return (
        "timed out" in msg or "connection refused" in msg or "couldn't connect" in msg
    )
...
def _is_unsupported_response_format(exc: Exception) -> bool:
    ...
    return any(
        tok in text
        for tok in ["unsupported", "unknown", "must be", "json_schema", "json object"]
    )
...
def _is_unsupported_reasoning_controls(exc: Exception) -> bool:
    ...
    if not mentions:
        return False
    return any(
        tok in text for tok in ["unsupported", "unknown", "unexpected", "invalid"]
    )
